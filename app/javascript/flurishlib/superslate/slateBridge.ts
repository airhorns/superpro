...
// const allowedOperations = ["insert_text", "remove_text", "insert_node", "split_node", "remove_node", "merge_node", "set_node", "move_node"];

// const applySlateOperationHelper = (automergeValue: any, slateOperation: Operation) => {
//   if (allowedOperations.indexOf(slateOperation.type) === -1) {
//     return;
//   }

//   const { path, offset, text, node, position, properties, newPath } = slateOperation.data.toJSON();
//   const index = path.get(path.size - 1);
//   const rest = path.slice(0, -1);
//   let currentNode = automergeValue.note;

//   switch (slateOperation.type) {
...
//         // Do nothing
//       } else if (oldParentPath.every((x, i) => x === newParentPath.get(i)) && oldIndex < newParentPath.get(oldParentPath.size)) {
//         // Remove the old node from it's current parent.
...
// export const applySlateOperations = (automergeDocument: any, path: string, slateOperations: Operation[]) => {
//   return Automerge.change(automergeDocument, doc => {
//     const slateDocument: ValueProperties = get(doc, path);
//     slateOperations.forEach(operation => {
//       applySlateOperationHelper(slateDocument, operation);
//     });
//   });
// };
