class Mutations::Budget::UpdateBudget < Mutations::BaseMutation
  ...
  def resolve(id:, budget:)
    existing_budget = context[:current_account].budgets.kept.includes(:budget_lines => [:account, :creator, :series, { :fixed_budget_line_descriptor => :budget_line_scenarios }]).find(id)
