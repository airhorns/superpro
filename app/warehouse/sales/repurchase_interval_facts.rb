...
class Sales::RepurchaseIntervalFacts < DataModel::FactTable
  ...
  measure :early_repurchase_rate, DataModel::Types::Number, allow_operators: false do
    Arel.sql("case when #{sql_string(table_node[:order_seq_number].eq(1))} and #{sql_string(table_node[:days_until_next_order].lteq(60))} then 1 end").count / nullif(Arel.sql("*").count, Arel.sql("0"))
  end
  measure :overall_repurchase_rate, DataModel::Types::Number, allow_operators: false do
    Arel.sql("case when #{sql_string(table_node[:order_seq_number].eq(1))} and #{sql_string(table_node[:days_until_next_order].not_eq(nil))} then 1 end").count / nullif(Arel.sql("*").count, Arel.sql("0"))
  end
